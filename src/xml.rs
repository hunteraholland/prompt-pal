...
impl XmlGenerator {
    ...
    fn generate_tree_view_recursive(node: &FileNode, prefix: &str, name: &str, result: &mut String) {
        if !name.is_empty() {
    ...
    fn build_tree(files: &[FileInfo]) -> FileNode {
        ...
        for file in files {
            let path_components: Vec<_> = file.path.components().map(|c| c.as_os_str().to_string_lossy().into_owned()).collect();
            let mut current = &mut root;
            
            // Create the path hierarchy
            for (i, component) in path_components.iter().enumerate() {
                let is_last = i == path_components.len() - 1;
                current = current.children.entry(component.clone()).or_insert_with(|| FileNode {
